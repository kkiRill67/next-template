...
В дополнение к <span className="spanTag">fetch()</span> на стороне клиента, Next.js полифицирует <span className="spanTag">fetch()</span> в среде Node.js. Вы можете использовать <span className="spanTag">fetch()</span> в коде вашего сервера (например, <span className="spanTag">getStaticProps</span>) без использования полифиллов, таких как <span className="spanTag">isomorphic-unfetch</span> или <span className="spanTag">node-fetch</span>.
...
Next.js имеет встроенную поддержку TypeScript.

<h3>
Настройка Babel Config (Дополнительно)
</h3>
Вы можете настроить конфигурацию babel.
            </div>
        </Docs>
    )
}