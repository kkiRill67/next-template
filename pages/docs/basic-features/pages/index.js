...
<p>Next.js поддерживает страницы с динамическими маршрутами. Например, если вы создадите файл с именем <span className="spanTag">pages/posts/[id].js</span>, он будет доступен по адресу <span className="spanTag">posts/1</span>, <span className="spanTag">posts/2</span> и т.д.</p>
...
<p>Если страница использует статическое создание, HTML-код страницы создается во время сборки. Это означает, что в производственной среде HTML страницы создаются при запуске <span className="spanTag">next build</span>. Затем этот HTML-код будет повторно использоваться для каждого запроса. Его можно кэшировать с помощью CDN.</p>
...
<p className='list'>
    1. Содержание вашей страницы зависит от внешних данных: Используйте <span className="spanTag">getStaticProps</span>.
</p>
<p className='list'>
    2. Пути к вашим страницам зависят от внешних данных: Используйте <span className="spanTag">getStaticPaths</span>(обычно в дополнение к <span className="spanTag">getStaticProps</span>).
</p>
...
<div className='note'>Чтобы узнать больше о динамической маршрутизации, обратитесь к документации по динамической маршрутизации.</div>
<p>Однако то, что <span className="spanTag">id</span> вы хотите предварительно отрисовать во время сборки, может зависеть от внешних данных.</p>
<p><strong>Пример:</strong> предположим, что вы добавили <span className="spanTag">id: 1</span> в базу данных только одно сообщение в блоге. В этом случае вам нужно выполнить предварительный рендеринг только <span className="spanTag">posts/1</span> во время сборки.</p>
<p>Позже вы можете добавить второй пост с расширением <span className="spanTag">id: 2</span>. Тогда вы также захотите выполнить предварительный рендеринг <span className="spanTag">posts/2</span>.</p>
<p>Таким образом, ваши пути к страницам, которые предварительно отображаются, зависят от внешних данных. Чтобы справиться с этим, Next.js позволяет вам <span className="spanTag">export</span> в <span className="spanTag">async</span> функцию с именем <span className="spanTag">getStaticPaths</span> из динамической страницы (<span className="spanTag">pages/posts/[id].js</span> в данном случае). Эта функция вызывается во время сборки и позволяет указать, какие пути вы хотите предварительно визуализировать.</p>
...
Кроме того <span className="spanTag">pages/posts/[id].js</span>, вам нужно экспортировать, <span className="spanTag">getStaticProps</span> чтобы вы могли получить данные о публикации с этим <span className="spanTag">id</span> и использовать их для предварительного рендеринга страницы:
...
<p>Как видите, <span className="spanTag">getServerSideProps</span> похож на <span className="spanTag">getStaticProps</span>, но с той разницей, что <span className="spanTag">getServerSideProps</span> запускается при каждом запросе, а не во время сборки.</p>
<p>Чтобы узнать больше о том, как работает <span className="spanTag">getServerSideProps</span>, ознакомьтесь с документацией по извлечению данных.</p>

<h2>Резюме</h2>
<p>Мы обсудили две формы предварительного рендеринга для Next.js.</p>
<p className="list"> - <strong>Статическая генерация (рекомендуется):</strong> HTML-код создается во время сборки и будет повторно использоваться при каждом запросе. Чтобы страница использовала статическое создание, либо экспортируйте компонент страницы, либо экспортируйте <span className="spanTag">getStaticProps</span>(и <span className="spanTag">getStaticPaths</span> при необходимости). Это отлично подходит для страниц, которые могут быть предварительно обработаны до запроса пользователя. Вы также можете использовать его с рендерингом на стороне клиента для ввода дополнительных данных.
</p>
<p className="list"> - <strong>Рендеринг на стороне сервера:</strong> HTML создается для каждого запроса. Чтобы страница использовала рендеринг на стороне сервера, выполните экспорт <span className="spanTag">getServerSideProps</span>. Поскольку рендеринг на стороне сервера приводит к снижению производительности, чем статическое создание, используйте его только в случае крайней необходимости.
</p>
