...
<p className='list'>
 - Иногда вам может потребоваться принудительно сбросить состояние и перемонтировать компонент. Например, это может быть удобно, если вы настраиваете анимацию, которая происходит только при монтировании. Для этого вы можете добавить в <span className="spanTag">// @refresh reset</span> любое место редактируемого файла. Эта директива является локальной для файла и предписывает <span className="spanTag">Fast Refresh</span> перемонтировать компоненты, определенные в этом файле, при каждом редактировании.
</p>
<p className='list'>
 - Вы можете вставлять <span className="spanTag">console.log</span> или вставлять <span className="spanTag">debugger</span>; компоненты, которые редактируете во время разработки.
</p>


<h2>Быстрое обновление и хуки</h2>
<p>
По возможности <span className="spanTag">Fast Refresh</span> пытается сохранить состояние вашего компонента между изменениями. В частности, <span className="spanTag">useState</span> и <span className="spanTag">useRef</span> сохранить их предыдущие значения до тех пор, пока вы не измените их аргументы или порядок вызовов хуков.
</p>
<p>
...
<p>
Например, при редактировании <span className="spanTag">useMemo(() => x * 2, [x])</span>в <span className="spanTag">useMemo(() => x * 10, [x])</span>, она будет повторно запустить, даже если x (зависимость) не изменилась. Если бы React этого не делал, ваше редактирование не отражалось бы на экране!
</p>
<p>
