...
<p className='list'>
 - Используйте <span className="spanTag">.env.local</span> для загрузки переменных среды
</p>
<p className='list'>
...
Если вы пытаетесь использовать переменную с <span className="spanTag">$</span> в реальной стоимости, он должен быть экранирован так: <span className="spanTag">\$</span>.
...
Это <span className="spanTag">process.env.NEXT_PUBLIC_ANALYTICS_ID</span> автоматически загружается в среду Node.js. Позволяя вам использовать его в любом месте вашего кода. Значение будет встроено в JavaScript, отправленный в браузер из-за <span className="spanTag">NEXT_PUBLIC_</span> префикса.
...
<div className='note'>
    <strong>Примечание:</strong> <span className="spanTag">.env</span>, <span className="spanTag">.env.development</span> и <span className="spanTag">.env.production</span> файлы должны быть включены в хранилище, поскольку они определяют по умолчанию. <span className="spanTag">.env*.local</span> должны быть добавлены <span className="spanTag">.gitignore</span>, поскольку эти файлы предназначены для игнорирования. <span className="spanTag">.env.local</span> здесь можно хранить секреты.
</div>
...
<p>
Помимо <span className="spanTag">development</span> и <span className="spanTag">production</span> среды, доступен третий вариант :<span className="spanTag">test</span>. Таким же образом вы можете установить значения по умолчанию для среды разработки или производства, вы можете сделать то же самое с <span className="spanTag">.env.test</span> файлом для среды тестирования (хотя этот не так распространен, как два предыдущих).
</p>
<p>
...
<p>
Существует небольшая разница между <span className="spanTag">test</span> окружающей средой, и как <span className="spanTag">development</span> и <span className="spanTag">production</span> что вы должны иметь в виду: <span className="spanTag">.env.local</span> не будет загружен, как вы ожидаете тесты дают одинаковые результаты для всех. Таким образом, каждое выполнение теста будет использовать одни и те же значения по умолчанию для разных исполнений, игнорируя ваш <span className="spanTag">.env.local</span>(который предназначен для переопределения набора по умолчанию).
</p>
<div className="note">
