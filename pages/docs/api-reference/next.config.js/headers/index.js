...
<p>Чтобы установить собственные заголовки HTTP, вы можете использовать <span className="spanTag">headers</span> ключ в next.config.js:</p>
...
<span className="spanTag">headers</span> это функция, которая ожидает асинхронной массив должен быть возвращен держа объекты с <span className="spanTag">source</span> и <span className="spanTag">headers</span> свойствами:
...
Если два заголовка соответствуют одному и тому же пути и задают один и тот же ключ заголовка, последний ключ заголовка будет иметь приоритет над первым. Используя приведенные ниже заголовки, путь /hello приведет к тому, что заголовок x-hello world будет соответствовать последнему заданному значению заголовка world.
...
При использовании <span className="spanTag">basePath</span> поддержки с заголовками каждый sourceавтоматически получает префикс, <span className="spanTag">basePath</span> если вы не добавляете <span className="spanTag">basePath: false</span> в заголовок:
