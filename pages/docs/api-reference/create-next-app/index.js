...
<h2>Параметры</h2>
create-next-app поставляется со следующими опциями:
...
<p className="list"> - <strong>--example-path [path-to-example]</strong> - В редких случаях ваш URL-адрес GitHub может содержать имя ветки с косой чертой (например, bug / fix-1) и путь к примеру (например, foo / bar). В этом случае необходимо указать путь к примеру отдельно:--example-path foo/bar</p>

<h2>Зачем использовать приложение Create Next?</h2>

create-next-app позволяет создать новое приложение Next.js за секунды. Он официально поддерживается создателями Next.js и имеет ряд преимуществ:
<p className="list"> - Интерактивный опыт : npx create-next-appзапуск (без аргументов) запускает интерактивный интерфейс, который поможет вам настроить проект.</p>
...
<p className="list"> - Поддержка примеров : Create Next App может загружать ваше приложение, используя пример из коллекции примеров Next.js (например npx create-next-app --example api-routes).</p>
