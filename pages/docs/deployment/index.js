...
Давайте поговорим о рабочем процессе, который мы рекомендуем использовать. Vercel поддерживает то, что мы называем <span className="spanTag">DPS</span> рабочего процесса: Develop, Preview, Ship:
...
Используя рабочий процесс <span className="spanTag">DPS</span>, вы можете не только выполнять обзоры кода, но и выполнять предварительные просмотры развертывания. Каждое развертывание создает уникальный URL-адрес, который можно использовать совместно или для интеграционных тестов.
...
<p>Убедитесь, что ваш <span className="spanTag">package.json</span> имеет <span className="spanTag">"build"</span> и <span className="spanTag">"start"</span> скрипты:</p>
...
<span className="spanTag">next build</span> создает производственное приложение в папке <span className="spanTag">.next</span>. После сборки <span className="spanTag">next start</span> запускает сервер Node.js, который поддерживает гибридные страницы, обслуживая как статически сгенерированные, так и отображаемые на стороне сервера страницы.
